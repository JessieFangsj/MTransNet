...
def show_on_report(batch_idx, num_batches, loss, elapsed):
    ...
    clear_line()
    dec = int(np.ceil(np.log10(num_batches)))
    print('Batch {:>{dec}d} / {:d} | Avg loss: {:>1.5f} | Avg train_NH time / batch: {:d} ms'.format(batch_idx + 1,
                                                                                                     num_batches, loss,
                                                                                                     int(elapsed),
                                                                                                     dec=dec))
...
def plot_per_epoch(ckpt_dir, title, measurements, y_label):
    """Plots stats (train_NH/valid loss, avg PSNR, etc.)."""
...
def validation_PSNR(net, val_data_loader, device, category, save_tag=False):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        torchvision.utils.save_image(dehaze,'../data/NH-Haze/MC-Net/false/output/{}.jpg'.format(batch_id))
...
def generate_filelist(img_dir, valid=False):
    ...
    filelist_name = 'val_list.txt' if valid else 'trainlist.txt'
    if len(filenames) == 1:
        if str(valid) == 'False':
            m = '/haze'
            filenames = []
            filenames = find_image(img_dir+m)
            # print(filenames)
            ...
            s = '/input'
            filenames = find_image(img_dir+s)
            with open(os.path.join(img_dir, filelist_name), 'w') as f:
                for i in range(len(filenames)):
                    f.write(filenames[i] + '\n')

def generate_filelist1(img_dir):
    # get filenames list
    filenames = find_image(img_dir+'/input')
    filelist_name = 'val_list.txt'
    with open(os.path.join(img_dir, filelist_name), 'w') as f:
        for i in range(len(filenames)):
            f.write(filenames[i] + '\n')
